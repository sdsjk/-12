/*! appcan v0.1.5 |  from 3g2win.com */
appcan.define("tab", function(a, b, c) {
    var d = '<div class="ub ub-f1" style="font-size:0.9375em" data-index="<%=index%>">    <div class="ub-f1 ub ub-ver item ub-ac sc-text-tab">    <%if(option.hasIcon){%><div class="ub-f1 fa <%=data.icon%> fa-2x tx-c"></div><%}%>    <%if(option.hasLabel){%><div class=" tx-c"><%=data.label%></div><%}%>    </div>    <%if(option.hasBadge && data.badge){%><div class="ulev headColor sc-bg-alert tab_badge" id="<%="badge_"+index%>"><%=data.badge%></div><%}%>    </div>',
    ...
    f.prototype = {
        focusAnim : function(b, c) {
            ...
            if (d.option.hasAnim) {
                ...
                d.focus = d.focus || a('<div class="focus utra bc-head"style="bottom: -1px;"></div>'), d.focus.css("width", f), d.focus.css("-webkit-transform", "translateX(" + b * f + "px)"), c && c.append(d.focus)
            }
        },
