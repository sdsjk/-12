...
(function ($) {
    ...
    function add(element, events, fn, data, selector, delegator, capture) {
      ...
      if ('addEventListener' in element) {
        var appcan_isAndroid = (window.navigator.userAgent.indexOf('Android') >= 0) || (window.navigator.userAgent.indexOf('Linux') >= 0);
        if(appcan_isAndroid) {
            var ec = eventCapture(handler, capture) || {}
            if(!ec.passive){
                ec.passive = false
            }
            element.addEventListener(realEvent(handler.e), handler.proxy, ec)
        } else {
...
(function ($) {
    ...
    $(document).ready(function () {
		...
		var appcan_isAndroid = (window.navigator.userAgent.indexOf('Android') >= 0) || (window.navigator.userAgent.indexOf('Adr') >= 0);
        if(appcan_isAndroid) {
    ...
    function setValue(key, val) {
        try {
            if (storage) {
                ...
                if (uexWindow.removeLocalData) {                    
                    uexWindow.putLocalData(key, val);
                } else {                    
    ...
    function getValue(key) {
        ...
        try {
            if (storage) {
                ...
                if (uexWindow.removeLocalData) {
                    var appcanval = uexWindow.getLocalData(key);                                       
                   if(appcanval){                     
                       return appcanval;
                   }
                }
    ...
    function clear(key) {
        try {
            if (key && appcan.isString(key)) {
                if (uexWindow && uexWindow.removeLocalData) {
            ...
            if (session.success) {
                ...
                switch (session.dom[0].tagName.toLowerCase()) {
                    case "img":
                        ...
                        break;
                    default:
                        if (fpath && session.dom.css("background-image").indexOf(fpath) < 0)
...
function getCurrentJSFileName() {
    var currentJSFileName, scripts = document.getElementsByTagName("script");
    currentJSFileName = scripts[scripts.length - 1].getAttribute("src");
    return currentJSFileName;
}
...
var $domPullRefresh = '#pullrefresh';//默认下拉加载dom对象

/**
 * 初始化下拉刷新和上拉加载
 *
 * @param callbackPulldownRefresh 下拉刷新回调
 * @param callbackPullupLoading 上拉加载回调
 * @param domPullRefresh 下拉刷新控件的dom对象，不传则默认为#pullrefresh
 */
function initPullRefresh(callbackPulldownRefresh, callbackPullupLoading, domPullRefresh) {

    var down, up;
    $domPullRefresh = '#pullrefresh';

    if (domPullRefresh) {
        $domPullRefresh = domPullRefresh;
    }
    if (callbackPulldownRefresh) {
        //下拉刷新配置
        down = {
            ...
            callback: callbackPulldownRefresh //必选，下拉刷新回调
        };
    }
    if (callbackPullupLoading) {
        //上拉加载配置
        up = {
            ...
            callback: callbackPullupLoading //必选，上拉加载回调
        };
    }
    ...
    mui.init({
        pullRefresh: {
            container: $domPullRefresh,
            down: down,
            up: up
        }
    });

    //一开始就调用上拉加载，如果没有这句话，会出现拉不动的情况
    if (mui.os.plus) {
       
        mui.plusReady(function () {
            setTimeout(function () {
                mui($domPullRefresh).pullRefresh().pullupLoading();
            }, 1000);
        });
    } else {
...
function pulldownRefresh() {
    mui($domPullRefresh).pullRefresh().pulldownLoading();
}
...
function endPulldownToRefresh() {
    mui($domPullRefresh).pullRefresh().endPulldownToRefresh(); //refresh completed
}
 ...
 * 结束上拉加载
 *
 * @param nomore 是否还有更多数据；若还有更多数据，则传入false; 否则传入true，之后滚动条滚动到底时，将不再显示“上拉显示更多”的提示语，而显示“没有更多数据了”的提示语
 */
function endPullupToLoading(nomore) {
    mui($domPullRefresh).pullRefresh().endPullupToRefresh(nomore); //参数为true代表没有更多数据了。
}
