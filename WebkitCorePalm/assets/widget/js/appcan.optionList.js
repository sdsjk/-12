...
var model_hiddenLine =  '<li style="position: relative;border-bottom:1px solid #EEE;color: black;display: -webkit-box !important;display: box !important;-webkit-box-align: center;box-align: center; padding:0;"  <%if(group){%>data-group="<%=group%>"<%}%> <%if(data.id){%>id="<%=data.id%>"<%}%> data-index="<%=index%>">\
...
<%}%>\
    ...
    OptionListView.prototype = {
        buildListview : function(data,group) {
            ...
            $(document).off("touchstart", ".scroll-bar").on("touchstart", ".scroll-bar", function(evt) {
                if(bounceState == 1){
            ...
            $(document).off("touchmove", ".scroll-bar").on("touchmove", ".scroll-bar", function(evt) {
            ...
            $(document).off("touchend", ".scroll-bar").on("touchend", ".scroll-bar", function(evt) {
                if(bounceState == 1){
                    appcan.window.enableBounce();
                }
                if (!scrollEnable)
            ...
            $(document).off("touchcancel", ".scroll-bar").on("touchcancel", ".scroll-bar", function(evt) {
                if(bounceState == 1){
                    appcan.window.enableBounce();
                }
                if (!scrollEnable)
        ...
        add : function(data, dir) {
            ...
            if(uexWindow.getBounce && typeof uexWindow.getBounce === 'function'){
                appcan.window.getBounceStatus(function(dt,dataType,opId){
                    ...
                    //点击右边隐藏部分执行回调
                    $(".scroll-right",container).off("tap").on("tap",function(e){
                        var that = this;
