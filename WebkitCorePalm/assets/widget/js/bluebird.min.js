...
/*
 * 接口统一调用方法
 * 参数释义：工程名称 接口名称  请求类型 接口参数 用户ID 页面名称 发起请求应用ID
 */

function allAppMasIn(projectName,interfaceName,method,json,userId,pageName,requestAppId){
    var token = appcan.getLocVal("emmAccessToken");
    
    var config_masUrl = config.mas_Server + config.appId + "/" + token + "/allAppMasIn/unified";
    
    var json = typeof json == "string" ? json : JSON.stringify(json); 
    ...
    var paramTrackTimerStart = {
        event:"requestDuration"
    };
    try{
        var spareUserId = JSON.parse(appcan.getLocVal("EPortal-UserInfo")).userId; // 用户ID
    }catch(e){
        var spareUserId = "测试";
    };
    
    var paramTrackTimerEnd = {
        event:"requestDuration",
        propertieDict:{
            userId:userId || spareUserId,   // 用户ID
            notesType:"接口请求",           // 记录类型
            pageName:pageName,             // 页面名称
            requestAppId:requestAppId,     // 发起请求应用ID
            requestType:projectName,       // MAS工程名
            requestInterface:interfaceName   // 请求接口
        }
    };
    ...
    return new Promise(function(resolve,reject){
        // 神策插件记录接口调用开始时间
        try{uexSensorsAnalytics.trackTimerBegin(JSON.stringify(paramTrackTimerStart));}catch(e){};
        $.ajax({
            url :config_masUrl,
            type : 'POST',
            dataType : 'json',
            ...
            success : function(data) {
                ...
                // 神策插件记录接口调用结束时间
                try{uexSensorsAnalytics.trackTimerEnd(JSON.stringify(paramTrackTimerEnd));}catch(e){};
...
//统一MAS接口传输数据加密方式
function allMasIsCrypto(projectName,interfaceName,method,json,userId,pageName,requestAppId){
    var token = appcan.getLocVal("emmAccessToken");
    
    var config_masUrl = config.mas_Server + config.appId + "/" + token + "/allAppMasIn/isCrypto";
    ...
    var bizData={};
    var biz={};
    bizData.projectName=projectName;
    bizData.interfaceName=interfaceName;
    bizData.method=method;
    bizData.json=json;
    ...
    var planeText = CryptoJS.enc.Utf8.parse(bizData);
    
    planeText = CryptoJS.AES.encrypt(planeText,key, {  
            mode: CryptoJS.mode.ECB,  
            padding: CryptoJS.pad.Pkcs7  
        });
    planeText = planeText.ciphertext.toString();
    
    biz.bizData=planeText;  
